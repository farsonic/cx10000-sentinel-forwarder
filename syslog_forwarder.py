...
# Set DEBUG using environment variable (default to False)
DEBUG = os.environ.get("DEBUG", "False").lower() in ("true", "1", "yes")

# ===== Configuration options via environment variables =====
WORKSPACE_ID = os.environ.get("WORKSPACE_ID", "default_workspace_id")
SHARED_KEY = os.environ.get("SHARED_KEY", "default_shared_key")
LOG_TYPE = os.environ.get("LOG_TYPE", "PensandoFlowLog")
MAPPORT = os.environ.get("MAPPORT", "True").lower() in ("true", "1", "yes")
MAXMIND_ENABLED = os.environ.get("MAXMIND_ENABLED", "True").lower() in ("true", "1", "yes")
...
# ===== PRINT REDIS STATISTICS PERIODICALLY =====
def redis_stats_printer():
    while True:
        if REDIS_AVAILABLE:
            try:
                success = redis_client.get("sentinel:logs:success")
                failure = redis_client.get("sentinel:logs:failure")
                success = int(success) if success else 0
                failure = int(failure) if failure else 0
                print(f"[REDIS STATS] Success: {success}, Failure: {failure}")
            except Exception as e:
                print(f"[REDIS STATS] Could not fetch counters: {e}")
        else:
            print("[REDIS STATS] Redis is not available.")
        time.sleep(60)
                ...
                if MAXMIND_ENABLED:
                    ...
                    parsed_log.update({
                        **src_geo,
                        ...
                        "geo_src_longitude": src_geo.get("geo_src_longitude", src_geo["geo_longitude"]),
                        **dst_geo,
                        "geo_dst_country": dst_geo["geo_country"],
                        "geo_dst_city": dst_geo["geo_city"],
                        "geo_dst_latitude": dst_geo["geo_latitude"],
                        "geo_dst_longitude": dst_geo.get("geo_dst_longitude", dst_geo["geo_longitude"])
                    })
...
if __name__ == "__main__":
    # Start a background thread to print Redis counters every minute
    stats_thread = threading.Thread(target=redis_stats_printer, daemon=True)
    stats_thread.start()
